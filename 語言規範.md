...
- **內置錯誤型別 error**  
  Glux 內置一個錯誤型別 **error**，用於表示錯誤狀態與錯誤訊息，通常與 union 搭配使用來實現錯誤處理。

- **集合型別支援異質性**  
  集合型別（如 tuple、list、array、map）不需要指定內部元素型別，允許同時存放不同型別的值。
- **內建函式 `string()`**  
...
### 5.7 函數回傳型別與多值
- **多值回傳**：函數可透過 tuple 同時返回多個值，所有回傳值均存在於一個 tuple 中。
...
- **錯誤處理與替代型別**：若函數回傳結果可能屬於成功型別 T 或錯誤型別 E（此處錯誤型別為內置錯誤型別 error），則可使用 union<T, error> 表示。為了簡化錯誤檢查，Glux 提供了內置函式 **is_error()**。使用者可直接利用 is_error() 快速判斷函數是否發生錯誤，當結果為非錯誤時，該 union 值可直接視為 T 使用，無需額外 unwrap()。
...
- **值傳遞**：對於基本型別（如 int、float、bool），預設採用值傳遞。這些型別佔用記憶體小，複製成本低。
...
- **copy()**：提供一個內置函式，用於對集合型別、結構體或其他複合型別進行深拷貝。  
  拷貝後返回一個與原物件不共享記憶體的新物件。
...
- **error()**：內置函式，用於生成一個 error 型別的錯誤物件。  
  通常接受一個錯誤訊息字串作為參數，以便在函數中返回錯誤狀態時使用。
...
✅ **內置 error 型別與 error() 函式用於錯誤處理，並配合 is_error() 進行快速檢查**  
