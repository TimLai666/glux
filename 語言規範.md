...
- 支援預設參數，且 **無預設值的參數必須放在有預設值的參數之前**；函數呼叫時允許使用關鍵字方式（例如 `param=value`）指定參數。
- 函式可作為變數、參數與回傳值。
...
println(greet("Hello"))                  // 使用預設值，輸出 "Hello, Guest"
println(greet("Hello", name="Alice"))      // 使用關鍵字參數，輸出 "Hello, Alice"
```
...
- **函數參數預設值**：
  - 允許函數參數具有預設值，提供預設值的參數在呼叫時可省略，但無預設值的參數必須放在有預設值參數之前。
  - 呼叫函數時，允許使用關鍵字形式指定參數（例如 `name="Alice"`）。
...
- **支持預設值**：結構體成員可以設定預設值，當建立實例時若未指定則使用預設值。

#### 範例
```rust
struct User {
    Name: string = "Unknown",  // 預設值 "Unknown"
    age: int,                  // 無預設值，必須指定
}
user := User { age: 30 }
println(user.Name)  // 輸出 "Unknown"
```

#### 9.1.1 Struct 繼承
- Glux 統一使用 `extends` 來實現結構體繼承，不支持嵌入。
- 若子結構與父結構有同名屬性，必須使用關鍵字 **override** 明確標示覆寫；否則編譯器報錯。
...
struct Employee extends Person {
    override Name: string,  // 覆寫父類 Name
    employeeId: int,
}
...
- 介面支持透過 `extends` 繼承其他介面，合併多個介面的功能。
...
- **error()**：生成一個 error 型別的錯誤物件，通常接受一個錯誤訊息字串作為參數，用於錯誤處理。
  ```rust
...
✅ **內置 copy() 支援深拷貝**  
✅ **支援 struct 與 interface，並提供繼承/擴展功能**  
   - Struct 使用 `extends` 實現繼承，並使用 **override** 關鍵字明確覆寫同名屬性。
   - Interface 可使用 `extends` 擴展其他介面，組合多個介面的方法簽名。
✅ **函數回傳型別可利用 tuple 與 union 進行多值回傳與錯誤處理**  
   - 使用 tuple 表示同時返回多個值（例如 `(int, int)`）。
   - 使用 union 表示回傳結果可能是成功型別 T 或錯誤型別 error（例如 `union<(int, string), error>`），並保持與 tuple 回傳寫法一致。
...
✅ **函數參數可有預設值，無預設值的參數必須放在有預設值參數之前；支持使用關鍵字形式呼叫函數**  
