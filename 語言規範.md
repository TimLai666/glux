...
- **錯誤處理與替代型別**：若函數回傳結果可能屬於成功型別 T 或錯誤型別 E（此處錯誤型別為內置 error），則可使用 union<T, error> 表示。例如，若要回傳多個值且可能出錯，可這樣定義：
  ```rust
  fn try_parse_division(s: string) -> union<(int, string), error> {
      // 此處回傳一個 tuple，其第一個元素為 int，第二個元素為 string
      if s == "42" {
          return (42, "成功")
      } else {
          return error("無法解析")
...
- **引用傳遞**：對於複合型別（如 string、list、array、map、tuple、struct 等），預設採用引用傳遞，以節省記憶體與提高效能；函式呼叫中傳遞的是物件的引用，而非整個複製品。
- 若有需要，可透過顯式語法指定值傳遞或引用傳遞，以滿足特殊需求。
...
✅ **函數回傳型別可利用 tuple 與 union 進行多值回傳與錯誤處理**  
   ...
   - 使用 union 表示回傳結果可能是成功型別 T 或錯誤型別 error（例如 `union<(int, string), error>`），且函數回傳 union<T, error> 時，可利用內置 is_error() 快速檢查錯誤，正常情況下 union 值可直接當作 T 使用。
