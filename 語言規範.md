...
- **錯誤處理與替代型別**：若函數回傳結果可能屬於多種型別中的一種，則可使用 union<T1, T2> 表示。例如，用於錯誤處理時，可以定義類似 Rust 的 Result<T, E>：
  ```rust
  fn try_parse_int(s: string) -> union<int, string> {
      if s == "42" {
          return 42
      } else {
          return "無法解析為整數"
  ...
  // 接下來需要使用模式匹配或類似機制來處理 union 型別
  ```
  
這樣的設計能夠靈活而精確地描述函數的回傳結構，既能同時返回多個值，也能表示成功與錯誤的替代結果。
